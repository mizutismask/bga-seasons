...
var CardStock = /** @class */ (function () {
    ...
    CardStock.prototype.cardRemoved = function (card) {
        ...
        index = this.selectedCards.findIndex(function (c) { return _this.manager.getId(c) == _this.manager.getId(card); });
        if (index !== -1) {
...
/**
 * A scrollable stock that handles sprite backgrounds for cards.
 */
var SpriteScrollableStock = /** @class */ (function (_super) {
    __extends(SpriteScrollableStock, _super);
    /**
     * @param manager the card manager
     * @param element the stock element (should be an empty HTML Element)
     * @param settings a `ScrollStockSettings` object
     */
    function SpriteScrollableStock(manager, elementWrapper, settings) {
        var _this = this;
        _this = _super.call(this, manager, elementWrapper, settings) || this;
        return _this;
    }

    /*  SpriteScrollableStock.prototype.addCard = function (card, animation, settings) {
          var promise = _super.prototype.addCard.call(this, card, animation, settings);
          //this.setBackgroundPosition(card, settings);
          return promise;
      };
  
      SpriteScrollableStock.prototype.setBackgroundPosition = function (card, settings) {
          if (toint(settings.imagePosition) !== 0) {
              var img_dx = 0;
              var img_dy = 0;
              var item_div = this.getCardElement(card);
              console.log("item_div", item_div);
             // dojo.style(item_div, "backgroundImage", "url(" + settings.backgroundImage + ")");
  
              if (settings.imageItemsPerRow) {
                  // Several rows
                  var row = Math.floor(settings.imagePosition / this.image_items_per_row);
  
                  img_dy = (settings.imagePosition - (row * this.image_items_per_row)) * 100;
                  img_dx = row * 100;
                  ...
                  // All in one row
                  img_dx = settings.imagePosition * 100;
                  dojo.style(item_div, "backgroundPosition", "-" + img_dx + "% 0%");
              }
  
          }
      }*/
    return SpriteScrollableStock;
}(ScrollableStock));
define({
    ...
    SpriteScrollableStock: SpriteScrollableStock,
